...
import { Starter } from '../../../../../../src/client/components/game/init'
...
describe('<Starter />', () => {
  ...
  it('clicks a player and selects a position', async () => {
    const player = playerConfigs.pitcher
    const positions = ['P', 'C']
  ...
  it('handles clearing a player out', async () => {
    ...
    const { container, getByText, getByDisplayValue } = renderInTable(
      <Starter
        index={0}
        players={Object.values(playerConfigs)}
        selectPlayer={selectPlayer}
        selectPosition={selectPosition}
      />
    )
